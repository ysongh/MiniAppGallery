import { arbitrum, base, celo } from "wagmi/chains";

export const getContractAddress = (networkId: number): `0x${string}` => {
  if (networkId === 84532) return import.meta.env.VITE_BASESEPOLIA_ONTRACT_ADDRESS as `0x${string}`;
  else if (networkId === 8453) return import.meta.env.VITE_BASE_ONTRACT_ADDRESS as `0x${string}`;
  else if (networkId === 44787) return import.meta.env.VITE_CELOALFAJORES_CONTRACT_ADDRESS as `0x${string}`;
  else if (networkId === 11142220) return import.meta.env.VITE_CELOSEPOLIA_CONTRACT_ADDRESS as `0x${string}`;
  else if (networkId === 42220) return import.meta.env.VITE_CELO_CONTRACT_ADDRESS as `0x${string}`;
  else if (networkId === 42161) return import.meta.env.VITE_ARBITRUM_CONTRACT_ADDRESS as `0x${string}`;
  return import.meta.env.VITE_LOCAL_CONTRACT_ADDRESS as `0x${string}`;;
}

export const supportedNetworks = [
  { id: base.id, name: 'Base', chain: base },
  // { id: baseSepolia.id, name: 'Base Sepolia', chain: baseSepolia },
  { id: arbitrum.id, name: 'Arbitrum', chain: arbitrum },
  // { id: celoSepolia.id, name: 'Celo Sepolia', chain: celoSepolia },
  { id: celo.id, name: 'Celo', chain: celo },
];